...
class H2Adapter2Spec extends AnyWordSpec with Matchers {
  ...
  "should found schema by name" in {
    ...
    adapter(DiscoverSchema("hello2")) should be (SchemaDiscovered("hello2", Set("SAMPLE", "EXAMPLE")))
  }
  ...
  "should discover table by name and schema" in {
    ...
    adapter.tables(Count()) should be (Counted(1))

    adapter(DiscoverTable("example", "hello2")) should be (
    ...
    adapter.tables(Count()) should be (Counted(2))
  }

  "should discover references of schema" in {
    adapter(DiscoverReferences("HELLO2")) should be (
    ...
    adapter.refs(Count()) should be (Counted(1))
  }
}
