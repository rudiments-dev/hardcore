...
case class MemoryNode(
  ...
  branches: mutable.Map[ID, MemoryNode] = mutable.Map.empty
) extends AgentCrud {
        ...
        this ? head match {
          case Readen(m: MemoryNode) =>
            ...
            path.ids.dropRight(1).foreach { id =>
              p = p / id
              m.unsafeUpdateState(p, Created(MemoryNode.empty))
            }
...
object MemoryNode {
  def empty: MemoryNode = MemoryNode(Nothing)
  def wrap(l: Location, t: Thing): MemoryNode = (l, t) match {
    case (Root, _: MemoryNode) => throw new IllegalArgumentException("Memory node in root is forbidden")
    case (Root, thing) => MemoryNode(thing)
    case (id: ID, mem: MemoryNode) => MemoryNode(branches = mutable.Map(id -> mem))
    case (id: ID, thing) => MemoryNode(leafs = mutable.Map(id -> thing))
    case (path: Path, _) =>
      MemoryNode(branches = mutable.Map(path.ids.head -> wrap(path -/ path.ids.head, t)))
