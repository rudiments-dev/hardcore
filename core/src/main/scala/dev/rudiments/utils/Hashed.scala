...
sealed trait Hashed(hash: Array[Byte]) {
  lazy val bigInteger: BigInteger = new BigInteger(1, hash)
...
final case class SHA1(hash: Array[Byte]) extends Hashed(hash) {
...
final case class SHA256(hash: Array[Byte]) extends Hashed(hash) {
  override def equals(obj: Any): Boolean = obj match {
...
object SHA256 {
  ...
  def fromHex(hex: String): SHA256 = new SHA256(Hashed.hexFormat.parseHex(hex))
}


final case class SHA3(hash: Array[Byte]) extends Hashed(hash) {
  override def equals(obj: Any): Boolean = obj match {
...
object SHA3 {
  ...
  def fromHex(hex: String): SHA3 = new SHA3(Hashed.hexFormat.parseHex(hex))
}
