...
class SoftCacheSpec extends WordSpec with Matchers {
  ...
  "update item in repository" in {
    cache(Update(id, SoftInstance(42, "sample", Some("changes")))).merge should be (
      Updated(
        id,
        SoftInstance(42, "sample", None),
        SoftInstance(42, "sample", Some("changes"))))
    cache(Count).merge should be (Counted(1))
    cache(Find(id)).merge should be (Found(id, SoftInstance(42, "sample", Some("changes"))))
  }

  "multiple inserts with same ID causes exception" in {
    cache(Count).merge should be (Counted(1))
    cache(Create(id, sample)).merge should be (AlreadyExists(id, SoftInstance(42, "sample", Some("changes"))))
  }

  "delete item from repository" in {
    cache(Count).merge should be (Counted(1))
    cache(Delete(id)).merge should be (Deleted(id, SoftInstance(42, "sample", Some("changes"))))
    cache(Count).merge should be (Counted(0))
    cache(Find(id)).merge should be (NotFound(id))
  }
