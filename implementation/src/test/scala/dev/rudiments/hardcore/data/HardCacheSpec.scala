...
class HardCacheSpec extends WordSpec with Matchers {
  ...
  "no element by ID" in {
    repo(Count[Example]()).merge should be (Counted(0))
    repo(Find(id)).merge should be (NotFound(id))
  }

  "put item into repository" in {
    repo(Count[Example]()).merge should be (Counted(0))
    repo(Create(id, sample)).merge should be (Created(id, sample))
    repo(Count[Example]()).merge should be (Counted(1))
    repo(Find(id)).merge should be (Found(id, sample))
  }

  "update item in repository" in {
    repo(Update(id, sample.copy(comment = Some("changes")))).merge should be (
      Updated(
        id,
        Example(42, "sample", MyEnum.Red),
        Example(42, "sample", MyEnum.Red, Some("changes"))))
    repo(Count[Example]()).merge should be (Counted(1))
    repo(Find(id)).merge should be (Found(id, Example(42, "sample", MyEnum.Red, Some("changes"))))
  }

  "multiple inserts with same ID causes exception" in {
    repo(Count[Example]()).merge should be (Counted(1))
    repo(Create(id, sample)).merge should be (AlreadyExists(id, Example(42, "sample", MyEnum.Red, Some("changes"))))
  }

  "delete item from repository" in {
    repo(Count[Example]()).merge should be (Counted(1))
    repo(Delete(id)).merge should be (Deleted(id, Example(42, "sample", MyEnum.Red, Some("changes"))))
    repo(Count[Example]()).merge should be (Counted(0))
    repo(Find(id)).merge should be (NotFound(id))
  }
  ...
  "endure 100.000 records" in {
    ...
    repo(Count[Example]()).merge should be (Counted(100000))

    val rnd = new Random().nextInt(100000)
    repo(Find(HardID(rnd))).merge should be (Found(HardID(rnd), Example(rnd, s"$rnd'th element", MyEnum.One)))
  }

  "endure 100.000 batch" in {
    val batch = (100001 to 200000).map(i => (HardID[Example, Long](i), Example(i, s"$i'th element", MyEnum.Two))).toMap
    repo(CreateAll(batch)).merge should be (AllCreated(batch))

    repo(Count[Example]()).merge should be (Counted(200000))

    val rnd = new Random().nextInt(200000)
    repo(Find(HardID[Example, Long](rnd))).merge should be (Found(
  ...
  "endure 100.000 replace" in {
    ...
    repo(ReplaceAll(batch)).merge should be (AllReplaced(batch))
  ...
  "clear repository" in {
    repo(Count[Example]()).merge should be (Counted(100000))
    repo(DeleteAll[Example]()).merge should be (AllDeleted[Example]())
    repo(Count[Example]()).merge should be (Counted(0))
  }
}
