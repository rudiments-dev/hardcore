...
class InstantiateFromScalaSpec extends WordSpec with Matchers {
  ...
  private val s1: Spec = typeSystem.makeFromScala[Spec, Sample1]
  private val c1: Spec = typeSystem.makeFromScala[Spec, Complicated1]
  private val c2: Spec = typeSystem.makeFromScala[Spec, Complicated2]
  private val c3: Spec = typeSystem.makeFromScala[Spec, Complicated3]


  private val enum = typeSystem.find[Abstract]("MyEnum")

  "can construct soft instance" in {
    val s = s1.fromProduct(typeSystem, Sample1(1, None, Set.empty))
  ...
  "can construct instance with plain field and it's composition" in {
    ...
    c1.fromProduct(typeSystem, complicatedPlain) should be (
      Instance(c1, Seq(
        42,
        Some(42L),
        Set("red", "green", "blue"),
  ...
  "can construct instance with nested type field and it's composition" in {
    ...
    val v = c2.fromProduct(typeSystem, complicatedComposite)
    c2.fromProduct(typeSystem, complicatedComposite) should be (Instance(c2, Seq(
  ...
  "can construct instance with enum field and it's composition" in {
    ...
    c3.fromProduct(typeSystem, complicatedEnum) should be (Instance(c3, Seq(
      typeSystem.afterParent(enum, "One"),
      Some(typeSystem.afterParent(enum, "Two")),
      Set(typeSystem.afterParent(enum, "Red"), typeSystem.afterParent(enum, "Two")),
      Map(
        "1" -> typeSystem.afterParent(enum, "One"),
        "2" -> typeSystem.afterParent(enum, "Two"),
        "red" -> typeSystem.afterParent(enum, "Red")
      )
    )))
  }
}
